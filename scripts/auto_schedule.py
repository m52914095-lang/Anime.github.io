...
GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY', os.environ.get('ai', ''))
...
MONITORED_FILE = os.path.join(os.path.dirname(__file__), '..', 'monitored_anime.json')
...
def check_anilist_new_episodes(anime_name):
    ...
    try:
        ...
        print(f"  AniList check error for {anime_name}: {e}")
    return None
...
def check_mal_anime(mal_id):
    ...
    try:
        ...
        print(f"  MAL check error for ID {mal_id}: {e}")
    return None
...
def get_streamp2p_uploaded_episodes(anime_name, api_key):
    ...
    try:
        ...
        print(f"  StreamP2P check error: {e}")
...
def trigger_download(anime_name, episode_range=None):
    ...
    if episode_range:
        print(f"  Would trigger: {cmd} (episodes: {episode_range})")
    else:
        print(f"  Would trigger: {cmd}")
    return cmd
...
def add_anime_to_monitor(anime_name, mal_id=None, anilist_id=None):
    ...
    for item in monitored:
        if item.get('name', '').lower() == anime_name.lower():
            print(f"  {anime_name} is already being monitored")
            return
    ...
    print(f"  Added {anime_name} to monitoring list")
...
def remove_anime_from_monitor(anime_name):
    ...
    save_monitored(monitored)
    print(f"  Removed {anime_name} from monitoring list")
...
def check_all_monitored():
    ...
    if not monitored:
        print("  No anime being monitored. Add anime with: python auto_schedule.py --add \"Anime Name\"")
        return
        ...
        if anilist_data:
            ...
            if status == 'RELEASING' and next_ep:
                ...
                if next_ep_num > last_checked + 1:
                    ...
                    print(f"    NEW EPISODES: {new_range}")
                    actions.append({
                        'anime': anime_name,
                        'action': 'download_new',
                ...
                if total_eps and total_eps > last_checked:
                    ...
                    print(f"    ANIME COMPLETED - remaining: {new_range}")
                    actions.append({
                        'anime': anime_name,
    ...
    if actions:
        ...
        for action in actions:
            print(f"    - {action['anime']}: {action['action']} ({action['episode_range']})")
...
def main():
    ...
    if args.add:
        ...
        monitored = load_monitored()
        if not monitored:
            print("  No anime being monitored.")
        else:
            print(f"\n  Monitored Anime ({len(monitored)}):")
            for idx, item in enumerate(monitored, 1):
                print(f"    {idx}. {item.get('name')} (Last checked ep: {item.get('last_episode_checked', 0)}, Status: {item.get('status', 'unknown')})")
    elif args.check:
        check_all_monitored()
    else:
        ...
        if specific:
            print(f"  Checking specific anime: {specific}")
