...
class StreamP2PUploader:
    ...
    def create_folder(self, name, parent_id=None, description=None):
        ...
        if res.status_code == 201:
            ...
            print(f"  Folder creation failed ({res.status_code}): {res.text[:200]}")
            return None
    ...
    def upload_file_tus(self, filepath, folder_id=None, progress_callback=None):
        ...
        try:
            ...
            if create_res.status_code not in (200, 201):
                print(f"  tus POST failed ({create_res.status_code}): {create_res.text[:300]}")
                return False
            ...
            if not upload_url:
                ...
                print(f"  tus POST missing Location header. Headers: {dict(create_res.headers)}")
                return False
            ...
            with open(filepath, 'rb') as f:
                while offset < file_size:
                    ...
                    if patch_res.status_code not in (200, 204):
                        print(f"  tus PATCH failed at offset {offset} ({patch_res.status_code}): {patch_res.text[:200]}")
                        # Try to resume
                        time.sleep(2)
                    ...
                    if progress_callback:
                        ...
                        print(f"    Progress: {pct:.1f}% ({offset / 1024 / 1024:.1f} / {file_size / 1024 / 1024:.1f} MB)")

        except Exception as e:
            print(f"  tus upload failed: {e}")
            return False
        ...
        if offset >= file_size:
            ...
            print(f"  Upload incomplete: {offset}/{file_size} bytes")
            return False
    ...
    def create_advance_upload(self, url, name=None, folder_id=None, selected=None):
        ...
        if res.status_code == 201:
            data = res.json()
            task_id = data.get('id')
            print(f"  Advance upload task created: {task_id}")
            return task_id
        else:
            print(f"  Advance upload failed ({res.status_code}): {res.text[:200]}")
            return None
    ...
    def upload_subtitle(self, video_id, subtitle_file, language='en', name=None):
        ...
        with open(subtitle_file, 'rb') as f:
            ...
            if res.status_code == 201:
                print(f"  Subtitle uploaded: {os.path.basename(subtitle_file)}")
                return res.json()
            else:
                print(f"  Subtitle upload failed: {res.status_code}")
                return None
...
def upload_processed_anime(uploader, processed_dir, anime_name):
    ...
    # Upload subtitles to videos
    subtitle_dir = os.path.join(processed_dir, '_subtitles')
    if os.path.isdir(subtitle_dir):
        print(f"\n  Uploading subtitles...")
        # Find uploaded videos and attach subtitles
        # This would require matching video IDs with subtitle files
        # For now, we'll skip this in batch mode
...
def upload_magnets_directly(uploader, magnet_data, anime_name):
    ...
    for task in results['tasks']:
        max_wait = 60  # Max 30 minutes per task
        waited = 0
        while waited < max_wait:
            try:
                status = uploader.check_advance_upload(task['task_id'])
                task_status = status.get('status', 'unknown')
                print(f"    Task {task['task_id']}: {task_status}")
        ...
        if waited >= max_wait:
            task['status'] = 'timeout'
            print(f"    Task {task['task_id']}: timed out waiting")
...
def main():
    if len(sys.argv) < 3:
        print("Usage: python upload_streamp2p.py <processed_dir> <anime_name>")
        sys.exit(1)
    ...
    if not STREAMP2P_API_KEY:
        print("Error: STREAMP2P_API_KEY not set!")
        sys.exit(1)

    print(f"\n{'='*60}")
    print(f"  StreamP2P Upload: {anime_name}")
    print(f"{'='*60}\n")
    ...
    if has_processed:
        ...
        if os.path.exists(magnet_file):
            ...
            print("No processed files or magnet links found!")
            sys.exit(1)
    ...
    print(f"  Soft sub: {len(results.get('softsub', []))} uploaded")
    print(f"  Hard sub: {len(results.get('hardsub', []))} uploaded")
    print(f"  Dub: {len(results.get('dub', []))} uploaded")
